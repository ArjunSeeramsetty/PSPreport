-- PostgreSQL 16 / Timescale requires a continuous aggregate to bucket the
-- hypertable partition key (ingested_at). Grid analytics need valid time, so
-- this refreshable materialized view projects current truth by report date.
