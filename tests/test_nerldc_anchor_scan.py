"""Unit tests for NERLDC monthly-anchor scanning."""
...
from psp_pipeline.quality.rldc_anchor_scan import (
    scan_nerldc_monthly_anchors,
    scan_rldc_monthly_anchors,
)
from psp_pipeline.storage.sqlite_curated_schema import ensure_curated_sqlite_schema


def test_scan_nerldc_monthly_anchors_empty_dir(tmp_path: Path) -> None:
    empty_dir = tmp_path / "empty"
    empty_dir.mkdir()
    db_path = tmp_path / "nerldc_test.sqlite"

    result = scan_nerldc_monthly_anchors(empty_dir, db_path)
    assert result["rldc"] == "nerldc"
    assert result["anchor_count"] == 0
    assert result["persisted_report_count"] == 0
    assert result["failed_reports"] == []


def test_scan_nerldc_monthly_anchors_skips_persisted(tmp_path: Path) -> None:
    input_dir = tmp_path / "reports"
    input_dir.mkdir()
    sample_pdf = input_dir / "NER-PSP-REPORT-DATED-15-04-2025.pdf"
    ...
    conn.executescript(
        ...
        VALUES ('nerldc', '2025-04-15', '{sample_pdf.name}', '{sample_pdf.resolve()}');
        """
    )
    conn.close()

    result = scan_rldc_monthly_anchors(input_dir, db_path, rldc="nerldc")
